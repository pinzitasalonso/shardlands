...
function render() {
  ...
  for (let ty = y0; ty <= y1; ty++) {
    for (let tx = x0; tx <= x1; tx++) {
      ...
      if (useSprites) {
        // the shared renderer paints the flat parts and hands scenery back
        groundSink.underworld = ty < 64;
        GroundRender.drawCell(ctx, tileAt, tx, ty, sx, sy, time, groundSink);
      } else {
